...
lwritelayers = 0                     # write subsurface layers to output [1] or not [0]. Use [0] for faster 2D simulations 
...
dz0 = 0.1                            # Size of uppermost grid cell (m)
...
rhosninit = 350.0                    # Density of initial snow pack (kg/m3)
rhosnprec = 350.0                    # Density of fresh snowfall (kg/m3)
...
T10m = -5                            # Measured 10 m snow temperature (C) 
dsnow_glob = 0.                      # Thickness of snow layer (1 year), at start or on 1 Jan. from acc in m over ice/firn surface
dfirn = 0.                           # Thickness of firn layer, (if ice deeper than zdeep, set to zdeep), at start or on 1 Jan. from acc in m over ice/firn surface
luseacc = 0                          # Use sonic data for: 0= not at all, 1=only for evaluation of ice melt and accumulation, 2= 1 + restrict accum based on precip with measured sonic altim.
...
surfangle = 1                        # parameter in slush formation routine : surface angle
...
lz0m = 4                             # Switch roughness length for momentum from file (0), set below (1) or a random value between zll and zul (2) or ice (3), or parameterized using Hmax based on Van Tiggelen et al (2021) (4) 
zll = 0.0005                         #	
zul = 0.05                           #
...
Hmax_info  = 1.0                     # Maximum height of ice obstacles, used in case lz0m = 4
...
lwmax = 315.1144, 3.4328,  -0.0325   # polynomial to describe upper limit of lwin as function of temperature (C), corresponding to max cloud cover
lwmin = 218.2815, 4.5129,   0.0400   # polynomial to describe lower limit of lwin as function of temperature (C), corresponding to min cloud 
