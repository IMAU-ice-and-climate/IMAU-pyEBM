...
lerrorgap = 1                        # jump over data gaps = 0 or interpolate = 1 or interpolate and use all data for averaging = 2 (errorflag)
lhourlysnowout = 1                  # write snow characteristics every [0] day or [1] hour, default is 0 because it slows down enormously, use only when hourly values are needed for evaluation
...
tstep = 30                         # Time step, choose small enough to keep subsurface calculations stable in case ImpExp = 1. In case ImpExp = 2, be mindful of the accuracy of implicit solver
dz0 = 0.05                           # Size of uppermost grid cell (m)
dzdeep = 1                           # Size of lowest  grid cell (m)
zdeep = 20                           # Depth of grid (m)
llake = 0                            # Switch to keep activate lake routine [0]
densice = 916.                       # ice density (kg/m3)
densfirn = 450.                      # firn density (kg/m3)
densclosure = 920.                   # firn pore closure density after which no liquid water is allowed to percolate (kg/m3)
rhosninit = 300.0                    # Density of initial snow pack (kg/m3)
rhosnprec = 300.0                    # Density of fresh snowfall (kg/m3)
...
T10m = -58.22                      # Measured 10 m snow temperature (C) 
dsnow_glob = 0.5                     # Thickness of snow layer (1 year), at start or on 1 Jan. from acc in m over ice/firn surface
dfirn = 20.                          # Thickness of firn layer, (if ice deeper than zdeep, set to zdeep), at start or on 1 Jan. from acc in m over ice/firn surface
...
zll = 0.0005                         #	
zul = 0.05                           #
z0msn = 0.0015                       # Roughness length for momentum for snow in case lz0m = 1 (m)
...
lz0h = 1                             # Switch roughness length for heat and moisture, 0.1*z0m (0), using Andreas 1987 parameterization (1), using Smeets and van den Broeke 2008 parameterization (2) or using Van Tiggelen et al (2021) parameterization (3). For smooth surfaces (z0m < 1mm) and lz0m > 0 Andreas 1987 is used.
...
lsnet = 2                         # switch calculate Snet from Sin and Sout (0), or calculate Snet and Sin based on Sout and running mean albedo (1) , or calculate Snet and Sout based on Sin and running mean albedo (2) 
albmin = 0.5                        # minimum albedo used to set error values to and prevent unrealistic values
albmax = 0.95                        # maximum albedo used to set error values to and prevent unrealistic values
emis = 0.98                             # Emissivity of the snow/ice surface 
